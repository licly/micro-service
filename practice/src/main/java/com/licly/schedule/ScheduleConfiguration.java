...
public class ScheduleConfiguration {
	...
	public static class DemoJob03Configuration {

		@Bean
		public JobDetail demoJob03() {
			return JobBuilder.newJob(DemoJob03.class)
					.withIdentity("demoJob03") // 名字为 demoJob02
					.storeDurably() // 没有 Trigger 关联的时候任务是否被保留。因为创建 JobDetail 时，还没 Trigger 指向它，所以需要设置为 true ，表示保留。
					.build();
		}

		@Bean
		public Trigger demoJob03Trigger() {
			// 简单的调度计划的构造器
			CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule("0/10 * * * * ? *");
			// Trigger 构造器
			return TriggerBuilder.newTrigger()
					.forJob(demoJob03()) // 对应 Job 为 demoJob02
					.withIdentity("demoJob03Trigger") // 名字为 demoJob02Trigger
					.withSchedule(scheduleBuilder) // 对应 Schedule 为 scheduleBuilder
					.build();
		}
